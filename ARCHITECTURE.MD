...
SCOS Order Management is a single-package backend API. Single product (SCOS Station P1 Pro), multi-warehouse fulfillment. Frontend lives in a separate repo (`scos-fe/`).

```
src/                Fastify + Prisma + Postgres — order verification/submission API
src/types/          API response types (e.g. OrderQuote) and shared type definitions
```
...
services            orderService.ts — orchestrates: lock stock → calculate → persist
...
Rules:
- Only repositories contain SQL or Prisma queries.
- The service owns the transaction (`prisma.$transaction`) and passes `tx` to repository methods that must join it.
- Repositories return domain types (`WarehouseWithStock`) or API response shapes from `src/types/api.ts`, so routes never see Prisma models.
...
- `OrderFulfillment` records the actual per-warehouse split (qty, distance, shipping cost) for a placed order, so the allocation decision made at order time is kept.
...
- `unit` (`pnpm test`) — `test/unit/domain/*`, pure function tests (allocation, pricing, orderCalculator) — no DB.
- `db` (`pnpm test:db`) — `test/api/*` route-level tests and `test/integration/orderService.test.ts`, which exercises the real transaction against Postgres, including row locking (the concurrency test checks each transaction saw the previous one's stock) and rollback. Files run serially because they share one database; `test/support/dbGuard.ts` requires a `*_test` database.

`pnpm test:integration` runs both against a disposable Postgres from `docker-compose.test.yml`. Seed data lives in `prisma/seedData.ts` and is reused by test fixtures and `test/support/db.ts`.
